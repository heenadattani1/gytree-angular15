...
export class NgOtpInputComponent implements OnInit, AfterViewInit {
  @Input() config: any = { length: 4 };
  // tslint:disable-next-line: no-output-on-prefix
  @Output() onInputChange = new EventEmitter<string>();
  @Input() formCtrl: FormControl;
	@Output() isKeyUpEnter = new EventEmitter();
  ...
  onKeyDown($event: any, inputIdx: any = null) {
  ...
  setValue(value: any) {
    ...
    if (!this.config.disableAutoFocus) {
      const containerItem: any = document.getElementById(`c_${this.componentKey}`);
  ...
  handlePaste(e: any) {
    ...
    let clipboardData: any = e.clipboardData || data['clipboardData'];
    if (clipboardData) {
